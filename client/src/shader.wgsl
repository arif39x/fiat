...
fn map(p: vec3<f32>) -> vec2<f32> {
    ...
    for (var i = 0u; i < loop_count; i = i + 1u) {
        ...
        let y = p.y;
        let z = p.z;
        let dist = sqrt(x*x + y*y + z*z) - 10.0;
...
fn axis_line(p: vec3<f32>) -> vec3<f32> {
    ...
    // X arrow: cone with tip at (AXIS_LEN, 0, 0), opening toward -x
    let xt = AXIS_LEN - p.x;
    ...
    // Y arrow: cone with tip at (0, AXIS_LEN, 0), opening toward -y
    let yt = AXIS_LEN - p.y;
    ...
    // Z arrow: cone with tip at (0, 0, AXIS_LEN), opening toward -z
    let zt = AXIS_LEN - p.z;
